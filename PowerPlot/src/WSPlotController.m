...
- (void)WS_handlePan:(UIPanGestureRecognizer *)sender;

/** Zoom in response to a pinch gesture. */
- (void)WS_handlePinch:(UIPinchGestureRecognizer *)sender;

/** Handle a single tap. */
- (void)WS_handleTap:(UITapGestureRecognizer *)sender;
...
- (void)WS_handlePinch:(UIPinchGestureRecognizer *)sender {
    ...
    if (((newxmaxD - newxminD) <= [self zoomRangeXD].rMax) &&
        ...
        // This fix is by Andrew Korepanov, 05/25/2012.
        NAFloat deltaYMin = [self scrollRangeYD].rMin - newyminD;
        NAFloat deltaYMax = [self scrollRangeYD].rMax - newymaxD;
        ...
        NAFloat deltaXMin = [self scrollRangeXD].rMin - newxminD;
        NAFloat deltaXMax = [self scrollRangeXD].rMax - newxmaxD;
