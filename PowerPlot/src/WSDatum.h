...
/// @brief A single datum representation.
///
/// This class describes a single datum point in a data set. The point
/// always has a value (Y-value) and may have an annotation, an
...
/// value will be an @p NSNumber whose @p floatValue corresponds to
/// the datum.
///
...
///
/// This class has two more slots whose use is optional. The first one
/// is called @p customDatum and can be any Cocoa object that
...
/// these customizations include @p WSPlotBar, @p WSPlotGraph etc. The
/// objects describing customizations typically are subclasses of @p
/// WSCustomProperties and their names end with @p Properties.
///
...
/// for the @p datum property if it is changed using any of the
/// 'set...'  methods listed below.
///
/// @note The convention for uncertainties is that whenever @p
///       errorMin[X|Y] exists, so will @p errorMax[X|Y] and vice
///       versa. If only one exists, errors are symmetric. If both
///       exist, errors are asymmetric.
@interface WSDatum : NSObject <NSCopying, NSCoding>

@property (strong) NSDictionary *datum;  ///< The dictionary containing the properties.
@property (strong) id <NSCoding, NSCopying, NSObject> customDatum; ///< Customization information.
@property (weak) id <NSObject> delegate; ///< Custom delegate.
...
/** @return An empty datum (factory method). */
+ (instancetype)datum;

/** @return Datum with a Y-value (factory method). */
+ (instancetype)datumWithValue:(NAFloat)aValue;

/** @return Datum with an annotated Y-value (factory method). */
+ (instancetype)datumWithValue:(NAFloat)aValue
                    annotation:(NSString *)anno;

/** @return Datum of a X,Y-pairs (factory method). */
+ (instancetype)datumWithValue:(NAFloat)aValue
                        valueX:(NAFloat)aValueX;

/** @return Datum of an annotated X,Y-pairs (factory method). */
+ (instancetype)datumWithValue:(NAFloat)aValue
                        valueX:(NAFloat)aValueX
                    annotation:(NSString *)anno;
...
/** @brief Alternate accessor for X-value.
    ...
    @return An @p NSDate instance based on the @p valueX property.
 */
- (NSDate *)dateTime;
...
/** @brief Compare the X-value of this datum with another X-value.
    ...
    @param aDatum @p WSDatum with which we compare the instance's
           properties.
