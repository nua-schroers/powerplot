...
- (void)drawRect:(CGRect)rect {
    ...
    WSData *allNodes = self.dataDelegate.dataD;
    for (WSConnection *item in ((WSGraph *)allNodes).connections) {

        // Get the relevant node properties.
        WSDatum *datumTo = allNodes[item.to];
        WSDatum *datumFrom = allNodes[item.from];
    ...
    for (NSInteger i=0; i<[allNodes count]; i++) {
        ...
        if (anno) {
            ...
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake((nodeRect.origin.x + padding),
                                                       (nodeRect.origin.y + padding),
                                                       (nodeRect.size.width - 2.0*padding),
                                                       (nodeRect.size.height - 2.0*padding))];
