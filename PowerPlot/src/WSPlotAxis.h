...
@property (strong, nonatomic) WSAxisProperties *axisX; ///< Style of X-axis.
@property (strong, nonatomic) WSAxisProperties *axisY; ///< Style of Y-axis.
...
@property (strong, nonatomic) WSTicks *ticksX;         ///< Tick marks on X-axis.
@property (strong, nonatomic) WSTicks *ticksY;         ///< Tick marks on Y-axis.
...
/** @brief Set the X-axis major ticks using a given data set.
    ...
    the chart, this method can simply be called with @p
    [&lt;instance&gt; dataD] as a parameter. Otherwise, the data
    (possibly from another data source) has to be supplied. This
    method does not use the plot's own data set on purpose.
  
    @param data @p WSData input data from which to take the X-values.
 */
...
/** @brief Set the major ticks and labels using a given data set.
    ...
    be called with @p [&lt;instance&gt; dataD] as a
    parameter. Otherwise, the data (possibly from another data source)
    has to be supplied. This method does not use the plot's own data
    set on purpose.
  
    @param data @p WSData input data from which to take the
           information.
 */
...
/** @brief Set the major ticks and labels using a given data set.
    ...
    @p [&lt;instance&gt; dataD] as a parameter. Otherwise, the data
    (possibly from another data source) has to be supplied as the
    parameter. This method does not use the plot's own data set on
    purpose.
...
/** @brief Set the X-axis major ticks automatically.
    ...
    chart. Another one is to use the methods in @p WSChart to keep the
...
/** @brief Set the X-axis ticks automatically to powers of ten of 1, 2
    and 5.
 
    This method provides a "nicer" way to set tick labels than the
    standard equi-distant method used above.
 
    @note This method also sets the tick labels to an appropriate
          number of digits!
 */
- (void)autoNiceTicksXD;

/** @brief Set the Y-axis ticks automatically to powers of ten of 1, 2
    and 5.
...
/** Set the X-tick labels using a default @p NSNumberFormatter
    style.
 */
- (void)setTickLabelsXWithStyle:(NSNumberFormatterStyle)style;

/** Set the Y-tick labels using a default @p NSNumberFormatter
    style.
 */
