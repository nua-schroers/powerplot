...
- (void)encodeWithCoder:(NSCoder *)encoder {
    [encoder encodeObject:[self connections] forKey:kConnectionsKey];
}

- (instancetype)initWithCoder:(NSCoder *)decoder {
    self = [super init];
    if (self) {
        _connections = [decoder decodeObjectForKey:kConnectionsKey];
    }
    return self;
}
...
- (NSString *)description {
    ...
    for (WSConnection *item in [self connections]) {
        ...
        if (i < descLen) {
            [prtCont appendFormat:@"(%lu,%lu,",
             (unsigned long)[item from],
             (unsigned long)[item to]];
