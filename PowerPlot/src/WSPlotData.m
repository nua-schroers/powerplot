...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [myColors addObject:(id)[aColor CGColor]];
        locs[1] = 1.0;
        _fillGradient = CGGradientCreateWithColors(mySpace, (CFArrayRef)myColors, locs);
...
- (void)setAllDisplaysOff {
    ...
    self.propDefault.symbolStyle = kSymbolNone;
    self.propDefault.errorStyle = kErrorNone;
    self.lineStyle = kLineNone;
    self.intStyle = kInterpolationStraight;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    *interpol2 = NULL;
    ...
    if (self.allowReordering) {
        sortedD = self.dataDelegate.dataD;
        if (!sortedD.isSorted) {
            [sortedD sortDataUsingValueX];
        }
    } else {
        sortedD = [self.dataDelegate.dataD sortedDataUsingValueX];
    }
    ...
    switch (self.intStyle) {
        ...
        case kInterpolationSpline: {
            CGPoint *source;
            unsigned long len = [sortedD count];
            ...
            for (NSUInteger i=0; i<len; i++) {
                source[i] = CGPointMake([self.coordDelegate boundsWithDataXD:sortedD[i].valueX],
                                        [self.coordDelegate boundsWithDataYD:sortedD[i].valueY]);
            }
    ...
    for (WSDatum *pointD in self.dataDelegate.dataD) {
        ...
        errorMaxY = -1.0;
        if ([pointD hasErrorX]) {
            // @note By convention, if @p errorMinX exists, so will @p errorMaxX.
        ...
        if ([pointD hasErrorY]) {
            // @note By convention, if @p errorMinY exists, so will @p errorMaxY.
